...
SUPPORTED_EXTS = [".pptx", ".pptm", ".potx", ".potm", ".ppsx", ".ppsm"]
FORMAT_NAME = "Microsoft PowerPoint Open XML (PPTX)"
...
def identify(hex_prefix: bytes, file_ext: str) -> int:
    ...
    """
    # 1. PPTX files are ZIP archives, check for the ZIP Local File Header signature 'PK\x03\x04'
    if len(hex_prefix) >= 30 and hex_prefix.startswith(MAGIC_BYTES):
        
        # 2. To distinguish PPTX from other ZIP-based formats, we look for Office Open XML structures.
        # The first few files in the archive typically include '[Content_Types].xml', '_rels/.rels', or 'ppt/'
        if b"ppt/" in hex_prefix:
            return 100  # 100% certainty that it is a PowerPoint-specific Open XML document
            
        if b"[Content_Types].xml" in hex_prefix:
            # It is definitely an Office Open XML file, check extension to confirm it's PowerPoint
            if file_ext.lower() in SUPPORTED_EXTS:
                return 100
            return 80 # Highly likely an Office document, but might be Word/Excel without the matching extension
            
        # 3. Magic bytes match a ZIP file, but we don't see obvious PPTX internal file paths in the first 4KB.
        # We rely on the extension to give a moderate confidence score.
        if file_ext.lower() in SUPPORTED_EXTS:
            return 60
            
        # Just a generic ZIP archive with no PPTX indicators
        return 20 
